...
class Courier {
  ...
  toJSON() {
    ...
    if(_.size(this._evtPatternHandlers)) obj.eventPattern = _.mapValues(this._evtPatternHandlers, v => _.map(v.handlers, v => _.pick(v, 'calls', 'lastCalled', 'once')));
  ...
  reply(name, handler, opts={}) {
    if(!opts.override && this._repHandlers[name]) throw new Error(`Reply handler already registered for "${name}"`);
  ...
  replyPattern(pattern, handler, opts={}) {
    if(!opts.override && this._repHandlers[pattern]) throw new Error(`Reply pattern handler already registered for "${pattern}"`);
...
class Context {
  ...
  unset() {
    this.data.unset.apply(this.data, arguments);
    return this;
  }
}
