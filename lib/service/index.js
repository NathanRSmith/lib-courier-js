...
class Service {
      ...
      .mapValues((v, k) => {
        try {
          ...
          if(v.namespace || v.namespaces) {
            ...
            courier.registerParent(this.courier);
          }
  ...
  _invokeModulesMethod(method, args=[]) {
    return Promise.props(_.mapValues(_.pickBy(this.modules, method), (v, k) => {
      this.logger.info(`running method "${method}" on module "${k}"`);
