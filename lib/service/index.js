...
class Service {
  ...
  initialize(opts) {
    ...
    this._requirePath = opts.requirePath;
    this.modules = [];
  ...
  instantiateModules() {
          ...
          try {
            ...
            if(v.namespace || v.namespaces) {
              ...
              _courier = _.reduce(ns.slice(1), (a, v) => {
                return courier.registerNamespace(v, a, _.pick(v, 'retainPrefix'));
              }, courier.registerNamespace(ns[0], null, _.extend({courierOpts: {name: v.name || k}}, _.pick(v, 'retainPrefix'))));
            }
            ...
            if(v.method) {
              ...
              return _.invoke(mod, v.method, v.config, _courier, this);
            }
            return {id: k, group: group, module: mod(v.config, _courier, this)};
          }
          catch(err) {
    ...
    //       if(v.namespace || v.namespaces) {
    //         const ns = _.chain(v.namespace).concat(v.namespaces).filter().uniq().value();
    //         courier = _.reduce(ns.slice(1), (a, v) => {
    //           return this.courier.registerNamespace(v, a)
    //         }, this.courier.registerNamespace(ns[0]));
    //       }
    //
    //       this.logger.info(`instantiating module "${k}"`);
    //
    //       if(v.method) {
    //         if(!_.has(mod, v.method) || !_.isFunction(_.get(mod, v.method))) throw new Error(`Invalid method "${cong.method}"`);
