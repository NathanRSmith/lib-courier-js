...
class Module {
  ...
  loadRepliers() {
    ...
    _.each(repliers, this.registerReplier.bind(this));
  }
  registerReplier(mod, key) {
    if(mod.enabled === false) return;
    if(mod.handlers) _.each(mod, this.registerReplier.bind(this));
    else if(mod.pattern) this.courier.replyPattern(mod.pattern, (mod.handler || mod).bind(null, this));
    else this.courier.reply(mod.name || key, (mod.handler || mod).bind(null, this));
  }
  ...
  registerListener(mod, key) {
    if(mod.enabled === false) return;
    if(mod.handlers) _.each(mod, this.registerListener.bind(this));
    else if(mod.pattern) this.courier.onPattern(mod.pattern, (mod.handler || mod).bind(null, this));
    else this.courier.on(mod.name || key, (mod.handler || mod).bind(null, this));
  }
